///////////////////////////////////////////////////////////////////////
// Class:       T0recoCRTTrack
// Module Type: producer
// File:        T0recoCRTTrack_module.cc
// Description: It generates and associates a T0 object to TPC Tracks ussing CRT Tracks
...
class crt::T0recoCRTTrack : public art::EDProducer {
public:
  explicit T0recoCRTTrack(fhicl::ParameterSet const & p);
  // The destructor generated by the compiler is fine for classes
  // without bare pointers or other resource use.

  // Plugins should not be copied or assigned.
  T0recoCRTTrack(T0recoCRTTrack const &) = delete;
  T0recoCRTTrack(T0recoCRTTrack &&) = delete;
  T0recoCRTTrack & operator = (T0recoCRTTrack const &) = delete;
  T0recoCRTTrack & operator = (T0recoCRTTrack &&) = delete;
...
void crt::T0recoCRTTrack::produce(art::Event & evt)
{
  ...
  if( CRTTrackCollection.size()<fShowerCut_) {//A0 cut in showers
	...
	if( (ThetaDiffABS<fThetaCut_) && (PhiDiffABS<fPhiCut_) ){//C0 //Angular Cut
	  

	  if( (BC[1]<BT[1])  && (TC[1]>TT[1]) ){//C1 //cut in Y //TPCTrack within CRTTrack 
	    double avg = (TT[2]+BT[2])/2;
	    if( ( avg>TC[2] && avg<BC[2]  ) || ( avg>BC[2] && avg<TC[2] ) ){//C2 //cut in Z //TPCTrack within CRTTrack 
	      
	      int CRTTrack_T1_nsec = my_CRTTrack.ts1_ns + fHardDelay_; 
	      int CRTTrack_T0_nsec = my_CRTTrack.ts0_ns + fCRTT0off_;
	      
	      double crtT = (CRTTrack_T0_nsec - evt_timeGPS_nsec)*0.001;//in us from trigger time
	      //double crtT = CRTTrack_T1_nsec*0.001;//in us from trigger time //if Beam is ON
	      double dT = 4*0.001;//CRT resolution 
	      
	      //for this track, make T0 object and associations using time of CRT T0
		...
		if(TdiffT0_nsecABS<fGPSMatchW_){//E : cut in GPS Match <1us
		  ...
		  //for this flash, make the association
...
void crt::T0recoCRTTrack::beginJob()
{
  // Implementation of optional member function here.
}

void crt::T0recoCRTTrack::endJob()
{
  // Implementation of optional member function here.
}

void crt::T0recoCRTTrack::SortTrackPoints(const recob::Track& track, std::vector<recob::Track::Point_t>& sorted_trk)
{
...
DEFINE_ART_MODULE(crt::T0recoCRTTrack)
