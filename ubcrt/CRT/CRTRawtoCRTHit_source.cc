...
// Version: V03_00 || added half top option || second assigmnent depends on full ts0 seconds
...
void crt::CRTRawInputDetail::scale_buffer(){
  ...
  for(int j=0;j<MAXFEBNR;j++){ //loop over all planes
   ...
   for(int i=0;i<ev_counter_mac[j];i++){  
    ...
    if( (ev_counter_mac[j]>1*EVSPERFEB || ev_counter_scan[j]>1*EVSPERFEB) && found_ppsref == 0){
      if( ev_counter_scan[j]>1*EVSPERFEB) ev_counter_scan[j]=0;
      if( ev_counter_filter[j]>1*EVSPERFEB) ev_counter_filter[j]=0;
      printf("try to prevent crash\n");
      if(ready_to_scan==SCANBUF_READY_TO_FILL){
