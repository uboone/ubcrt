...
bool UBCRTCosmicFilter::filter(art::Event & e)
{
  ...
  for ( int j = 0; j < _nCRThits_in_event; j++ ) {
    ...
    // Reset the variables.                                                                                                                                                                           
    if ( fabs( _beam_flash_time - ( ( crthit_h->at( j ).ts0_ns - evt_timeGPS_nsec  + fDTOffset ) / 1000. ) ) < _dt_abs ) {
      _dt_abs       = fabs( _beam_flash_time - ( ( crthit_h->at( j ).ts0_ns - evt_timeGPS_nsec + fDTOffset ) / 1000. ) );
      _dt           = _beam_flash_time - ( ( crthit_h->at( j ).ts0_ns - evt_timeGPS_nsec + fDTOffset ) / 1000. );
      _CRT_hit_time = ( ( crthit_h->at( j ).ts0_ns - evt_timeGPS_nsec +  fDTOffset ) / 1000. );
