/**
 *  @file   larpandora/LArPandoraEventBuilding/CRTLArPandoraExternalEventBuilding.cc
 *
...
class CRTLArPandoraExternalEventBuilding : public art::EDProducer
{
public:
    explicit CRTLArPandoraExternalEventBuilding(fhicl::ParameterSet const & pset);
    
    CRTLArPandoraExternalEventBuilding(CRTLArPandoraExternalEventBuilding const &) = delete;
    CRTLArPandoraExternalEventBuilding(CRTLArPandoraExternalEventBuilding &&) = delete;
    CRTLArPandoraExternalEventBuilding & operator = (CRTLArPandoraExternalEventBuilding const &) = delete;
    CRTLArPandoraExternalEventBuilding & operator = (CRTLArPandoraExternalEventBuilding &&) = delete;
...
DEFINE_ART_MODULE(CRTLArPandoraExternalEventBuilding)
...
CRTLArPandoraExternalEventBuilding::CRTLArPandoraExternalEventBuilding(fhicl::ParameterSet const &pset) :
...
void CRTLArPandoraExternalEventBuilding::produce(art::Event &evt)
{
...
void CRTLArPandoraExternalEventBuilding::CollectPFParticles(const art::Event &evt, PFParticleToMetadata &particlesToMetadata, PFParticleVector &particles) const
{
    ...
    for (unsigned int i = 0; i < pfParticleHandle->size(); ++i)
    {
        ...
        if (metadata.size() != 1) 
            throw cet::exception("LArPandora") << " CRTLArPandoraExternalEventBuilding::CollectPFParticles -- Found a PFParticle without exactly 1 metadata associated." << std::endl;

        if (!particlesToMetadata.insert(PFParticleToMetadata::value_type(part, metadata.front())).second)
            throw cet::exception("CRTLArPandoraExternalEventBuilding") << "Repeated PFParticles" << std::endl;
    }
}

//------------------------------------------------------------------------------------------------------------------------------------------

void CRTLArPandoraExternalEventBuilding::BuildPFParticleMap(const PFParticleToMetadata &particlesToMetadata, PFParticleMap &particleMap) const
{
...
void CRTLArPandoraExternalEventBuilding::CollectClearCosmicRays(const PFParticleVector &allParticles, const PFParticleToMetadata &particlesToMetadata, const PFParticleMap &particleMap, PFParticleVector &clearCosmics) const
{
    for (const auto &part : allParticles)
    {
        ...
        if (parentIt == particlesToMetadata.end())
            throw cet::exception("CRTLArPandoraExternalEventBuilding") << "Found PFParticle without metadata" << std::endl;
...
void CRTLArPandoraExternalEventBuilding::CollectSlices(const PFParticleVector &allParticles, const PFParticleToMetadata &particlesToMetadata, const PFParticleMap &particleMap, SliceVector &slices) const
{
    ...
    for (unsigned int sliceId = 1; sliceId <= nuScores.size(); ++sliceId)
    {
        ...
        if (nuScoresIter == nuScores.end())
            throw cet::exception("CRTLArPandoraExternalEventBuilding") << "Scrambled slice information - can't find nuScore with id = " << sliceId << std::endl;
...
float CRTLArPandoraExternalEventBuilding::GetMetadataValue(const art::Ptr<larpandoraobj::PFParticleMetadata> &metadata, const std::string &key) const
{
    const auto &propertiesMap(metadata->GetPropertiesMap());
    const auto &it(propertiesMap.find(key));

    if (it == propertiesMap.end())
        throw cet::exception("CRTLArPandoraExternalEventBuilding") << "No key \"" << key << "\" found in metadata properties map" << std::endl;
...
void CRTLArPandoraExternalEventBuilding::CollectConsolidatedParticles(const PFParticleVector &allParticles, const PFParticleVector &clearCosmics, const SliceVector &slices, PFParticleVector &consolidatedParticles) const
{
